...
import { type DayReportMetrics } from "@shared/types";
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex justify-between text-xs text-muted-foreground">
                    ...
                    <span>Worked: {(pe!.workedMinutes / 60).toFixed(1)}h</span>
                    {pe!.trackedMinutes > 0 && <span>Tracked: {(pe!.trackedMinutes / 60).toFixed(1)}h</span>}
                  </div>
                </div>
              </div>
            </CardContent>
        ...
        <Card className={hasPlanExecution ? '' : 'md:col-span-2 lg:col-span-3'}>
          ...
          <CardContent>
            {hasTopApps ? (
              ...
              <p className="text-sm text-muted-foreground">No app data recorded</p>
            )}
          </CardContent>
        </Card>
      </div>

      {/* Today's Activity Timeline — always show */}
      <TimelineChart data={timelineData || { segments: [] }} />

      {/* Focus Flow Chart — always show */}
