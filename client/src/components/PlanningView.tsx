...
import { InsertSubtask, UpdateSubtask, UpdateTask, type InsertTask } from "@shared/schema";
import { SubtaskWithMetrics, TaskWithMetrics } from "@shared/metrics";
...
export default function PlanningView() {
  ...
  const handleCompleteSubtask = (subtaskId: number, isCompleted: boolean) => {
    updateSubtaskMutation.mutate(
      { id: subtaskId, updates: { isCompleted: !isCompleted } },
      {
        onSuccess: () => {
          toast({
            title: "Subtask Updated",
            description: `Subtask has been marked as ${!isCompleted ? 'completed' : 'incomplete'}`,
          });
        },
        onError: (error: any) => {
          toast({
